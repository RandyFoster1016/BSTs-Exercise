 ...
 if(val < current.val) {
  ...
  find(val) {
    ...
    while (currentNode && !found) {
      if (val < currentNode.val){
        ...
        cuurrentNode = currentNode.right;
      }else {
        found = true;
      }
    }

    if (!found) return underfined;
    return currentNode;
  }
  ...
  dfsPreOrder() {
    ...
    function traverse(node) {
      ...
      node.left && traverse(node.left);// go left if thers's a left
  ...
  dfsInOrder() {
    ...
    function traverse(node){
      ...
      data.push(node.val); // visit
      node.right && traverse(node.right); // go right if ther's a right
    }
  ...
  dfsPostOrder() {
    ...
    function traverse(node){
      node.left && traverse(node.left); // go left if ther's a left
      node.rigth && traverse(node.right); // go right if ther's a right
      data.push(node.val); // visit
    }
  ...
  isBalanced(current=this.root) {
    ...
    function minDepth(current){
      ...
      return 1 + Math.min(minDepth(current.left), minDepth(curren.right))
    }
  ...
  findSecondHighest(current = this.root) {
     ...
     while (current){
      // Current is largest and has a left subtree and 2nd largest is the
